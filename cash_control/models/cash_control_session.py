    ...
    def compute_previous_session(self):
        for session in self:
            session_id = self.search(
                [('config_id', '=', session.config_id.id), ('state', '=', 'closed'),
                 ('id', '<', session.id)
                 ],
            ...
            if session_id:
                ...
                session.previous_session_id = False

    @api.depends('statement_ids', 'payment_ids', 'statement_balance_start', 'statement_id')
    def _compute_cash_balance(self):
        for session in self:
            cash_payment_method = session.statement_id.journal_id
            total_cash_payment = sum(session.payment_ids.filtered(
                lambda payment: payment.journal_id == cash_payment_method).mapped('amount'))
            extra = 0.0 if session.state == 'closed' else total_cash_payment
            session.statement_total_entry_encoding = session.statement_id.total_entry_encoding
            session.statement_balance_end = session.statement_balance_start + \
                session.statement_total_entry_encoding
            session.statement_difference = session.statement_balance_end_real - \
    ...
    def create(self, vals):
        ...
        if 'name' in vals and 'pwodoo_sync' in self.env.context:
            config_id = self.env['cash.control.config'].search(
                [('id', '=', vals['config_id'])])
            vals['payment_journal_ids'] = [(4, x.id)
                                           for x in config_id.payment_journal_ids]

            vals['payment_summary_ids'] = [
                (0, 0, {'journal_id': journal.id}) for journal in config_id.payment_journal_ids]
        ...
        vals['payment_summary_ids'] = [
            (0, 0, {'journal_id': journal.id}) for journal in config_id.payment_journal_ids]
        statement = False
        for journal in config_id.payment_journal_ids.filtered(lambda journal: journal.type == 'cash'):
        ...
        if not statement:
            raise UserError(
                _('Su Caja no tiene ningun diario de efectivo asignado'))
    ...
    def _check_pos_session_balance(self):
        for session in self:
            for statement in session.statement_ids:
                if (statement != session.statement_id) and (statement.balance_end != statement.balance_end_real):
                    statement.write(
...
class CashControlSessionPaymentSummary(models.Model):
    ...
    def compute_amount(self):
        ...
        for session_id in session_ids:
            ...
            for sumary in self.filtered(lambda line: line.session_id.id == session_id.id):
